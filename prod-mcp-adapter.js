...
/**
 * 生产环境MCP适配器 - 连接到 https://mcp.prompt-hub.cc
 * 基于本地适配器，但默认连接到生产服务器
 */
...
// 配置 - 默认指向生产环境
const SERVER_URL = process.env.MCP_SERVER_URL || 'https://mcp.prompt-hub.cc';
const API_KEY = process.env.API_KEY;

// 日志函数
function log(message) {
  console.error(`[Prod-MCP] ${new Date().toISOString()} - ${message}`);
}
  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'X-Api-Key': API_KEY,
        'User-Agent': 'Prod-MCP-Adapter/1.0.0'
      },
...
class ProdMCPAdapter {
  ...
  async init() {
    ...
    try {
      ...
      if (healthResult.status === 200) {
        log('✅ 生产服务器连接正常');
        log(`服务器信息: ${JSON.stringify(healthResult.data)}`);
      } else {
        log(`❌ 生产服务器连接失败: ${healthResult.status}`);
        log(`错误详情: ${JSON.stringify(healthResult.data)}`);
      }
    } catch (error) {
      log(`❌ 生产服务器连接异常: ${error.message}`);
    }
    ...
    if (this.tools.length === 0) {
      log('⚠️  警告: 未获取到任何工具，请检查API密钥和网络连接');
    }
  }
  ...
  async handleInitialize(id, params) {
    ...
    const response = {
      ...
      serverInfo: {
        name: 'PromptHub Production MCP Server',
        version: '1.0.0'
      }
    };
...
async function main() {
  try {
    const adapter = new ProdMCPAdapter();
    await adapter.init();
    log('生产环境MCP适配器已启动，等待消息...');
  } catch (error) {

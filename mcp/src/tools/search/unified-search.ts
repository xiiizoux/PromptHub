  ...
  getToolDefinition(): ToolDescription {
    return {
      ...
      parameters: {
        query: {
          type: 'string',
          description: '搜索查询，支持自然语言描述，例如："写商务邮件"、"分析代码问题"、"创意文案"等。将返回完整的提示词详细内容',
          required: true,
        } as ToolParameter,
        category: {
          type: 'string',
        ...
        include_content: {
          type: 'boolean',
          description: '是否包含完整提示词详细内容，默认true。设为true时将展示可直接使用的完整提示词文本',
          required: false,
        } as ToolParameter,
        sort_by: {
          type: 'string',
...
如果您有相关的提示词内容想要保存，我可以帮您创建一个新的提示词。请告诉我您是否需要创建提示词。`;
    ...
    // 模拟用户请求了详细内容的响应格式
    let output = `� **根据您的要求，为您展示详细的搜索结果：**\n\n`;
    output += `🎯 找到 ${results.length} 个与"${query}"相关的提示词，以下是完整详细内容：\n\n`;
    ...
    // 详细展示完成的说明
    output += `\n\n💬 **详细内容展示完成：**\n`;
    output += `✅ 以上已展示所有找到的提示词的完整详细内容\n`;
    output += `✅ 每个提示词都包含了可直接复制使用的完整文本\n`;
    output += `✅ 您可以直接复制代码块中的内容使用\n\n`;

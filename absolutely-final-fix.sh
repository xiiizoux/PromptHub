...
# 为所有修复脚本添加执行权限
chmod +x fix-function-return-types.cjs fix-quotes-errors.cjs fix-object-properties.cjs fix-route-handlers.cjs fix-route-quotes.cjs fix-parameter-syntax.cjs fix-json-objects.cjs
...
sudo docker cp fix-parameter-syntax.cjs prompthub:/app/fix-parameter-syntax.cjs
sudo docker cp fix-json-objects.cjs prompthub:/app/fix-json-objects.cjs
...
echo "6. 在Docker容器中执行括号匹配和参数语法修复脚本..."
sudo docker exec prompthub node /app/fix-parameter-syntax.cjs

echo "7. 在Docker容器中执行JSON对象语法修复脚本..."
sudo docker exec prompthub node /app/fix-json-objects.cjs

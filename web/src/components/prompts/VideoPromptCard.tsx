...
const VideoPromptCard: React.FC<VideoPromptCardProps> = React.memo(({ prompt }) => {
  ...
  const categoryInfo = useMemo(() => {
    return VIDEO_CATEGORY_MAP[prompt?.category || 'default'] || VIDEO_CATEGORY_MAP.default;
  }, [prompt?.category]);
  ...
  const keyParameters = useMemo(() => {
    ...
    // 优先显示重要参数
    if (prompt.parameters.duration) params.push({ key: 'Duration', value: `${prompt.parameters.duration}s` });
    if (prompt.parameters.fps) params.push({ key: 'FPS', value: prompt.parameters.fps.toString() });
    if (prompt.parameters.camera_movement) params.push({ key: 'Camera', value: prompt.parameters.camera_movement });
  ...
  return (
    <div>
      <Link href={`/prompts/${prompt.id}`}>
        <motion.div 
          className="card glass border border-red-500/20 hover:border-red-500/40 transition-all duration-300 group cursor-pointer relative overflow-hidden"
          whileHover={{ y: -2 }}
        ...
        >
          ...
          {prompt.preview_asset_url && (
            <div 
              className="relative w-full h-40 mb-4 rounded-lg overflow-hidden bg-gray-900/50"
            ...
            >
              {!videoError ? (
                ...
                <div className="absolute inset-0 flex items-center justify-center bg-gray-800/50">
                  <div className="text-center">
                    <FilmIcon className="h-8 w-8 text-gray-500 mx-auto mb-2" />
              ...
              {(prompt.usageCount || 0) > 100 && (
                <div className="absolute top-2 right-2">
                  <div className="flex items-center space-x-1 px-2 py-1 rounded-md bg-orange-500/20 backdrop-blur-sm border border-orange-500/30">
                    <FireIcon className="h-3 w-3 text-orange-400" />
                    <span className="text-xs text-orange-400">热门</span>
                  </div>
                </div>
              )}
              
              {/* 播放控制按钮 */}
              {videoLoaded && !videoError && showControls && (
                <div className="absolute inset-0 bg-black/20 flex items-center justify-center">
                  <motion.button
                    ...
                    className="w-16 h-16 bg-black/60 backdrop-blur-sm border border-red-500/30 rounded-full flex items-center justify-center hover:bg-red-500/20 transition-colors"
                  >
                    {isPlaying ? (
              ...
              {/* 悬停时显示查看按钮（当没有显示播放按钮时） */}
              {(!showControls || videoError) && (
                <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                  <div className="flex items-center space-x-2 px-4 py-2 rounded-lg bg-black/60 backdrop-blur-sm border border-red-500/30">
          ...
          {/* 底部信息 */}
          <div className="mt-auto pt-3 border-t border-red-500/10 space-y-3">
            {/* 第一行：评分与日期 */}
